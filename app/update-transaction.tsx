...
import { formatCategoryName, formatDate, formatDateAPI, formatTime, getCurrentDate, getCurrentTime, getRandomInt } from '@/utils/helper';
import { createTransaction, updateTransaction } from './services/transactions';

export default function UpdateTransaction() {

    const { id, user_id, category_id, category_name, amount, type, remarks, description, transaction_date, created_at, timestamp } = useLocalSearchParams();
    ...
    const [categoryID, setCategoryID] = useState(category_id.toString())
    const [categoryName, setCategoryName] = useState(formatCategoryName(category_name.toString()))
    const [categories, setCategories] = useState<Categories[]>([]);

    const [inputDate, setDate] = useState(currentDate);
    const [inputTime, setTime] = useState(currentTime);
    const [inputAmount, setAmount] = useState(parseInt(amount.toString()));
    const [inputRemarks, setRemarks] = useState(remarks.toString());
    const [modalVisible, setModalVisible] = useState(false);
    const [showDatePicker, setShowDatePicker] = useState(false);
    const [showTimePicker, setShowTimePicker] = useState(false);

    const onChangeDatePicker = (event: any, selectedDate?: Date) => {
        let _currentDate = selectedDate || inputDate;
    ...
    const onChangeTimePicker = (event: any, selectedTime?: Date) => {
        let _currentTime = selectedTime || inputTime;
            ...
            try {
                ...
                try {
                    const response = await updateTransaction(
                        parseInt(id.toString()), formatDateAPI(inputDate) + " " + formatTime(inputTime), parseInt(categoryID), inputAmount, "Expense", inputRemarks
                    );
        ...
        <GestureHandlerRootView>
            <View className='flex flex-col bg-white'>
                <View className='flex flex-row items-center mt-8 pb-3 px-2 border-b-2 border-gray-500 bg-white'>
                    ...
                    <Text className='ml-4 text-xl font-bold'>Update Transaction #{}</Text>
                </View>
                <View className='flex flex-row items-end mt-4 px-3 '>
                    ...
                    <View className='grow mx-2'>
                        <View className='flex flex-row items-end'>
                            ...
                            <TextInput showSoftInputOnFocus={false} value={formatDate(inputDate)} onPress={() => { setShowDatePicker(true) }} className='col-start-1 grow col-span-4 font-bold border-b border-slate-400 w-max pb-[1px] pt-4 text-black ml-1'></TextInput>
                        </View>
                    </View>
                    <View className='grow mx-2'>
                        <View className='flex flex-row items-end'>
                            <MaterialIcons name="access-time" size={20} color="black" />
                            <TextInput showSoftInputOnFocus={false} value={formatTime(inputTime)} onPress={() => { setShowTimePicker(true) }} className='col-start-1 grow col-span-4 font-bold border-b border-slate-400 w-max pb-[1px] pt-4 text-black ml-1'></TextInput>
                        </View>
                    </View>
                </View>
                <View className='flex flex-row items-end mt-4 px-3 '>
                    <Text className='col-start-1 col-span-2 mr-2 w-24 text-slate-600'>Amount</Text>
                    <TextInput value={inputAmount.toString() == "0" || inputAmount.toString() == "" ? "0" : inputAmount.toString()} onChangeText={text => setAmount(parseInt(text == "0" || text == "" ? "0" : text))} className='col-start-1 grow col-span-4 font-bold border-b border-slate-400 w-max pb-[1px] pt-4 text-black'></TextInput>
                </View>
                <View className='flex flex-row items-end mt-4 px-3 '>
                ...
                <View className='flex flex-row items-end mt-4 px-3 '>
                    ...
                    <TextInput multiline numberOfLines={5} textAlignVertical="top" value={inputRemarks} onChangeText={text => setRemarks(text)} className='col-start-1 flex-1 text-wrap col-span-4 font-bold border-[1px] border-slate-400 h-[100px] text-black'></TextInput>
                </View>
